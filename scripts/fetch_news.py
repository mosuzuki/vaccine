...
def country_info_from_key(key, config):
    info = config.get("country_map", {}).get((key or "unknown").lower())
    if not info:
...
def dedupe_items(items):
    ...
    for i, item in enumerate(items):
        ...
        group = [item]
        used.add(i)
        title_a = normalize_space((item.get("title_original") or item.get("title") or "").lower())
        url_a = normalize_url(item.get("link", ""))
        for j in range(i + 1, len(items)):
            if j in used:
                continue
            other = items[j]
            title_b = normalize_space((other.get("title_original") or other.get("title") or "").lower())
            url_b = normalize_url(other.get("link", ""))
            same_url = (url_a and url_a == url_b)
            title_score = fuzz.token_set_ratio(title_a, title_b)
            if same_url or title_score >= 90:
                group.append(other)
                used.add(j)
        group = sorted(group, key=lambda x: (x.get("source_priority", 0), x.get("published_at", "")), reverse=True)
...
def build_empty_news(config, now_utc):
    return {
        ...
        "item_count": 0,
        "plot_mode_default": config.get("default_plot_mode", "source"),
        "feed_status": [],
        "items": []
    }


def has_core_vaccine_signal(text, config):
    text_l = (text or "").lower()
    required_terms = config.get("required_vaccine_terms", [])
    return any(term.lower() in text_l for term in required_terms)


def has_exclusion_signal(text, config):
    text_l = (text or "").lower()
    return any(term.lower() in text_l for term in config.get("exclude_terms", []))


def is_pickup_candidate(item, config):
    if item.get("source_tier") not in config.get("pickup_tiers", ["Tier 1", "Tier 3"]):
        return False
    if has_exclusion_signal(" ".join([item.get("title_original", ""), item.get("summary_original", "")]), config):
        return False
    if not has_core_vaccine_signal(" ".join([item.get("title_original", ""), item.get("summary_original", "")]), config):
        return False
    topics = set(item.get("topics", []))
    if not topics.intersection(set(config.get("purpose_topics", []))):
        return False
    if item.get("source_tier") == "Tier 3" and not (item.get("vaccines") or item.get("policy_tags") or "research" in item.get("topics", [])):
        return False
    return True
    ...
    for feed in feeds:
        ...
        try:
            ...
            for entry in parsed.entries:
                ...
                if not is_pickup_candidate(item, config):
                    continue
                all_items.append(item)
                kept_count += 1
            feed_status.append({"name": name, "url": url, "status": "ok", "seen": seen_count, "kept": kept_count, "tier": tier})
        except Exception as exc:
            feed_status.append({"name": name, "url": url, "status": "error", "error": str(exc), "seen": seen_count, "kept": kept_count, "tier": tier})

    all_items = dedupe_items(all_items)
    all_items = all_items[: int(config.get("max_items", 250))]
    news = {
        "title": config.get("title", "Vaccine and Immunization Monitoring"),
        "description": config.get("description", ""),
        "generated_at": now_utc.isoformat(),
